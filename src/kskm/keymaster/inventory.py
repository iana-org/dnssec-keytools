        ...
        for slot, session in sorted(module.sessions.items()):
            ...
            for this in module.get_key_inventory(session):
                ...
                if label_and_id in keys[this.key_class]:
                    logger.error(f'Key with class {this.key_class} and label+id {label_and_id} already seen in slot')
                    continue
...
def _format_keys(data: Dict[KeyClass, Dict[str, KeyInfo]], config: KSKMConfig) -> List[str]:
    res: List[str] = []
    pairs: List[str] = []
    ...
    if KeyClass.PUBLIC in data and KeyClass.PRIVATE in data:
        ...
        for label_and_id in initial_keylist:
            ...
            if label_and_id in data[KeyClass.PRIVATE]:
                pairs += [f'      {this.label:7s} {_id_to_str(this.key_id)}{str(this.pubkey)} -- {ksk_info}']
