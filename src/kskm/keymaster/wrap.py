...
def key_backup(
    label: str, wrap_label: str, p11modules: KSKM_P11
) -> Optional[WrappedKey]:
    ...
    if wrap_key.privkey_handle and existing_key.privkey_handle:
        _wrap = session.wrapKey(
            cast(int, wrap_key.privkey_handle[0]),
    ...
    if existing_key.key_type == KeyType.RSA and isinstance(
        existing_key.public_key, KSKM_PublicKey_RSA
    ):
        return WrappedKeyRSA(
            key_label=existing_key.label,
...
def key_restore(wrapped_key: WrappedKey, p11modules: KSKM_P11) -> bool:
    ...
    if not unwrap_key:
        logger.error(
            f"No secret key with label {repr(wrapped_key.wrap_key_label)} found"
        )
        return False
    ...
    logger.info(
        f"Restoring key {wrapped_key.key_label} using wrapping key {unwrap_key}"
    )
    session = get_session(p11modules, logger)
    ...
    privateKeyTemplate = private_key_template(
        wrapped_key.key_label, wrapped_key.key_type.value
    )
    res = session.unwrapKey(
        unwrap_key.privkey_handle[0],
        wrapped_key.private_wrapped,
        privateKeyTemplate,
    ...
    if isinstance(wrapped_key, WrappedKeyRSA):
        publicKeyTemplate = public_key_template(
            wrapped_key.key_label,
            wrapped_key.key_type.value,
