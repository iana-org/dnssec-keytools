...
class Test_KSK_Policy_Two_Bundles(Test_Requests_With_Two_Bundles):
    ...
    def test_zsk_policy_no_bundle_overlap(self):
        ...
        xml = self._make_request(request_policy=request_policy, bundle1=bundle1, bundle2=bundle2)
        request = request_from_xml(xml)
        ...
        with self.assertRaises(KSR_POLICY_SIG_OVERLAP_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle "test-2" does not overlap with previous bundle "test-1" (2019-02-01 00:00:00+00:00 > '
                         '2019-01-22 00:00:00+00:00)', str(exc.exception))
    ...
    def test_zsk_policy_min_bundle_overlap(self):
        ...
        with self.assertRaises(KSR_POLICY_SIG_OVERLAP_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle "id=test-2 2019-01-20->2019-02-12" overlap 2 days with '
                         '"id=test-1 2019-01-01->2019-01-22" is < claimed minimum 9 days', str(exc.exception))
    ...
    def test_zsk_policy_max_bundle_overlap(self):
        ...
        with self.assertRaises(KSR_POLICY_SIG_OVERLAP_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle "id=test-2 2019-01-05->2019-01-28" overlap 17 days with '
                         '"id=test-1 2019-01-01->2019-01-22" is > claimed maximum 12 days', str(exc.exception))
    ...
    def test_request_with_one_day_overlap_problem(self):
        ...
        with self.assertRaises(KSR_POLICY_SIG_OVERLAP_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle "id=test-2 2019-01-14->2019-02-04" overlap 8 days with '
                         '"id=test-1 2019-01-01->2019-01-22" is < claimed minimum 9 days', str(exc.exception))
    ...
    def test_wrong_number_of_bundles(self):
        ...
        with self.assertRaises(KSR_POLICY_KEYS_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Can\'t check number of keys per bundle for a KSR with 2 bundles', str(exc.exception))
    ...
    def test_wrong_total_number_of_keys(self):
        ...
        with self.assertRaises(KSR_POLICY_KEYS_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Unacceptable number of key sets in request test, (1 keys instead of 2)', str(exc.exception))
    ...
    def test_too_short_signature_validity(self):
        ...
        with self.assertRaises(KSR_POLICY_SIG_VALIDITY_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle validity 8 days < claimed min_signature_validity 21 days (in bundle test-2)',
                         str(exc.exception))
    ...
    def test_too_long_signature_validity(self):
        ...
        with self.assertRaises(KSR_POLICY_SIG_VALIDITY_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle validity 141 days > claimed max_signature_validity 21 days (in bundle test-2)',
                         str(exc.exception))
    ...
    def test_min_bundle_interval(self):
        ...
        with self.assertRaises(KSR_POLICY_BUNDLE_INTERVAL_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle #2 (test-2) interval (11 days) less than minimum acceptable interval 15 days',
                         str(exc.exception))
    ...
    def test_max_bundle_interval(self):
        ...
        with self.assertRaises(KSR_POLICY_BUNDLE_INTERVAL_Violation) as exc:
            validate_request(request, policy)
        self.assertEqual('Bundle #2 (test-2) interval (11 days) greater than maximum acceptable interval 9 days',
                         str(exc.exception))
