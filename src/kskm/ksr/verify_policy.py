...
def check_signature_horizon(request, policy, logger):
    ...
    for bundle in request.bundles:
        ...
        if (
            policy.signature_horizon_days
            and expire_days > policy.signature_horizon_days
        ):
            logger.error(
                f"Bundle {bundle.id} signature expires in {expire_days} days ({bundle.expiration}), "
                f"above maximum of {policy.signature_horizon_days}"
            )
            raise KSR_POLICY_SIG_HORIZON_Violation(
                "Bundle signature expires too far in the future"
            )
        ...
        if policy.signature_horizon_days > 0 and expire_days < 0:
            logger.error(
                f"Bundle {bundle.id} signature expires {abs(expire_days)} days in the past "
                f"({bundle.expiration})"
            )
            raise KSR_PolicyViolation("Bundle signature expire in the past")

    logger.info(
        f"KSR-POLICY-SIG-HORIZON: All signatures expire in less than {policy.signature_horizon_days} days"
    )


def check_zsk_policy_algorithm(
    request: Request, policy: RequestPolicy, logger: Logger
) -> None:
    """
    KSR-POLICY-ALG.
    ...
    for alg in request.zsk_policy.algorithms:
        ...
        if alg.algorithm not in SUPPORTED_ALGORITHMS:
            raise KSR_POLICY_ALG_Violation(
                f"Algorithm {alg.algorithm.name} not supported"
            )
    ...
    if not policy.signature_algorithms_match_zsk_policy:
        logger.warning(
            "KSR-POLICY-ALG: Disabled by policy (signature_algorithms_match_zsk_policy)"
        )
        return
    ...
    for alg in request.zsk_policy.algorithms:
        if alg.algorithm not in _approved_algorithms:
            raise KSR_POLICY_ALG_Violation(
                f"ZSK policy has {alg.algorithm}, but policy only allows "
    ...
    for alg in request.zsk_policy.algorithms:
        if is_algorithm_rsa(alg.algorithm):
            ...
            if alg.bits not in policy.rsa_approved_key_sizes:
                raise KSR_POLICY_ALG_Violation(
                    f"ZSK policy has RSA-{alg.bits}, but policy dictates "
    ...
    logger.info(
        f"KSR-POLICY-ALG: All {_num_algs} ZSK operator signature algorithms accepted by policy"
    )


def check_bundle_overlaps(
    request: Request, policy: RequestPolicy, logger: Logger
) -> None:
    """
